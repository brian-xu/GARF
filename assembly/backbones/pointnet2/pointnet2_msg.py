...
from .pointnet2_dynamic_utils import (
...
from ..pointtransformerv3.model import offset2batch
...
class PointNet2PTMSG(nn.Module):
    ...
    def __init__(self, feat_in=3, feat_out=2):
        ...
        self.sa1 = PointNetSetAbstractionMsg(
            1024, [0.05, 0.1], [16, 32], self.feat_in, [[16, 16, 32], [32, 32, 64]]
        )
...
class PointNet2PTMSGDynamic(nn.Module):
    ...
    def __init__(self, feat_in=3, feat_out=2):
        ...
        self.feat_in = feat_in
        self.feat_out = feat_out
        self.sa1 = PointNetSetAbstractionMsgDynamic(
            0.15, [0.05, 0.1], [16, 32], self.feat_in, [[16, 16, 32], [32, 32, 64]]
        )
        self.sa2 = PointNetSetAbstractionMsgDynamic(
            0.25, [0.1, 0.2], [16, 32], 32 + 64, [[64, 64, 128], [64, 96, 128]]
        )
        self.sa3 = PointNetSetAbstractionMsgDynamic(
            0.25, [0.2, 0.4], [16, 32], 128 + 128, [[128, 196, 256], [128, 196, 256]]
        )
        self.sa4 = PointNetSetAbstractionMsgDynamic(
            0.25, [0.4, 0.8], [16, 32], 256 + 256, [[256, 256, 512], [256, 384, 512]]
        )
    ...
    def forward(self, input_dict):
        ...
        piece_idx = batch.reshape(1, 1, -1)

        # idx = 0
        # piece_idx = []
        # for i in range(len(batch_length)):
        #     piece_idx.append(
        #         idx * torch.ones(batch_length[i], dtype=torch.int64).to(x.device)
        #     )
        #     idx += 1
        ...
        l3_points = self.fp4(
            l3_xyz, l4_xyz, l3_piece_id, l4_piece_id, l3_points, l4_points
        )
        l2_points = self.fp3(
            l2_xyz, l3_xyz, l2_piece_id, l3_piece_id, l2_points, l3_points
        )
        l1_points = self.fp2(
            l1_xyz, l2_xyz, l1_piece_id, l2_piece_id, l1_points, l2_points
        )
