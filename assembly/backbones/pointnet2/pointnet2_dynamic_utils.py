...
def square_distance_with_piece(src, dst, src_piece_id, dst_piece_id):
    ...
    indices = torch.where(
        src_piece_id.repeat(1, 1, M) == dst_piece_id.view(B, 1, M).repeat(1, N, 1)
    )
...
def query_ball_point(radius, nsample, xyz, new_xyz, piece_id, new_piece_id):
    ...
    if group_idx.shape[-1] < nsample:
        group_idx = torch.cat(
            [
                group_idx,
                N * torch.ones(size=[B, S, nsample - N], dtype=torch.long).to(device),
            ],
            dim=-1,
        )
...
class PointNetSetAbstractionMsgDynamic(nn.Module):
    ...
    def forward(self, xyz, piece_id, points):
        ...
        assert B == 1
        centroids = fps(
            xyz[0, :, :], batch=piece_id.reshape(B * N), ratio=self.ratio
        ).unsqueeze(0)
        S = centroids.shape[1]
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            group_idx = knn(
                ...
                new_xyz[0, :, :],
                k=K,
                batch_x=piece_id.reshape(-1),
                batch_y=new_piece_id.reshape(-1),
            )
            group_idx = to_dense_batch(
                group_idx[1], group_idx[0], fill_value=N, max_num_nodes=K
            )[0].unsqueeze(0)
...
class PointNetFeaturePropagationDynamic(nn.Module):
    ...
    def forward(self, xyz1, xyz2, piece_id1, piece_id2, points1, points2):
        ...
        if S == 1:
            ...
            idx = to_dense_batch(
                group_idx[1], group_idx[0], fill_value=-1, max_num_nodes=3
            )[0].unsqueeze(0)
            idx_first = idx[:, :, 0].view(B, xyz1.shape[1], 1).repeat([1, 1, 3])
            mask = idx == -1
            idx[mask] = idx_first[mask]
            dists = to_dense_batch(
                dists, group_idx[0], fill_value=1e8, max_num_nodes=3
            )[0].unsqueeze(0)
